...
void LCDCursorReset(unsigned short row_id, unsigned short column_id);
...
void LCDCursorReset(unsigned short p_2, unsigned short p_1) {
    
    LCDCmd(0x0E);   // cursor off
    ...
    /*
    unsigned short number_of_moves = (y - 1) * 20 + (x - 1);
    for (unsigned short i = 0; i < number_of_moves; i++)
        LCDCmd(LCD_CURSOR_MOVE_RIGHT);
    //LCDCmd(0x0E);
    /*
    LCDCmd(LCD_CURSOR_BLINK);
    LCDCmd(LCD_CURSOR_ON);*/
}
